...
public class AffichageMatrice {
	...
	public void resetGrid() {
		for (int i = 0; i < sudoCasePanel.length; i++) {
			for (int j = 0; j < sudoCasePanel.length; j++) {
				sudoCasePanel[i][j].remove(sudoLabel[i][j]);
				sudokuCase[i][j].setSelectedIndex(0);
				sudoCasePanel[i][j].add(sudokuCase[i][j]);
				sudoCasePanel[i][j].revalidate();
				sudoCasePanel[i][j].repaint();
			}
		}
		fenetre.revalidate();
	}
